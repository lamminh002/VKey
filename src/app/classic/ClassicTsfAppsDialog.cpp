...
bool ClassicTsfAppsDialog::Show(HINSTANCE hInstance, HWND parent, bool forceLightTheme) {
    ClassicTsfAppsDialog dlg;
...
bool ClassicTsfAppsDialog::Init(HINSTANCE hInstance, HWND parent, bool forceLightTheme) {
    ...
    hwnd_ = CreateWindowExW(WS_EX_TOPMOST, kClassName, L"Danh sách app dùng TSF",
    ...
    EnumChildWindows(hwnd_, [](HWND h, LPARAM lp) -> BOOL {
        auto* self = reinterpret_cast<ClassicTsfAppsDialog*>(lp);
...
void ClassicTsfAppsDialog::CreateControls() {
...
void ClassicTsfAppsDialog::PopulateList() {
...
void ClassicTsfAppsDialog::AddApp(const std::wstring& name) {
    ...
    if (lower == L"vkey.exe" || lower == L"vkeylite.exe" || lower == L"vkeyclassic.exe") {
        MessageBoxW(hwnd_, L"Không thể thêm VKey vào danh sách TSF.",
...
void ClassicTsfAppsDialog::DeleteSelected() {
...
void ClassicTsfAppsDialog::ImportFromFile() {
...
void ClassicTsfAppsDialog::ExportToFile() {
...
void ClassicTsfAppsDialog::OnPickWindow() {
...
void ClassicTsfAppsDialog::LoadData() {
    appList_ = ConfigManager::LoadTsfApps(ConfigManager::GetConfigPath());
}

void ClassicTsfAppsDialog::SaveData() {
    modified_ = true;
...
int ClassicTsfAppsDialog::Dpi(int value) const noexcept {
...
LRESULT CALLBACK ClassicTsfAppsDialog::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) try {
    ClassicTsfAppsDialog* self = nullptr;

    if (msg == WM_NCCREATE) {
        auto* cs = reinterpret_cast<CREATESTRUCTW*>(lParam);
        self = reinterpret_cast<ClassicTsfAppsDialog*>(cs->lpCreateParams);
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(self));
        self->hwnd_ = hwnd;
    } else {
        self = reinterpret_cast<ClassicTsfAppsDialog*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    }
    ...
    NextKey::CrashLog(L"ClassicTsfAppsDialog::WndProc", e.what());
    return DefWindowProcW(hwnd, msg, wParam, lParam);
} catch (...) {
    NextKey::CrashLog(L"ClassicTsfAppsDialog::WndProc", "(non-std exception)");
    return DefWindowProcW(hwnd, msg, wParam, lParam);
}
